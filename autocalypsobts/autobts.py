...
class CaptureWorker(QThread):
    ...
    def read_output(self):
        ...
        while self.process心里anReadLine():
...
class SubscribersDialog(QDialog):
    def __init__(self, subscribers_text, theme, parent=None):
        super().__init__(parent)
        self.setWindowTitle('SUBSCRIBERS')
        self.setModal(False)  # Non-modal
        self.resize(700, 500)  # Initial size
        layout = QVBoxLayout(self)
        
        text_edit = QTextEdit()
        text_edit.setReadOnly(True)
        text_edit.setPlainText(subscribers_text)
...
For Mini0comOS and DragonOS
    ...
    def get_subscribers(self):
        stream = os.popen('sudo python3 /usr/src/CalypsoBTS/sub.py')
        return stream.read()
    ...
    def click_button5(self):
        os.popen('sudo featherpad /usr/src/CalypsoBTS/openbsc.cfg')
    
    def click_button6(self):
        os.popen('sudo featherpad /usr/src/CalypsoBTS/osmo-bts-trx-calypso.cfg')
    
    def click_button7(self):
        stream = os.popen('sudo python3 /usr/src/CalypsoBTS/sub.py')
        out = stream.read()
        dialog = SubscribersDialog(out, self.current_theme, self)
        dialog.show()
    ...
    def handle_remove_db(self, button, msg_box):
        if button.text() == "&Yes":
            stream = os.popen('sudo rm  /usr/src/CalypsoBTS/hlr.sqlite3 >/dev/null 2>&1 & echo "REMOVED"')
            out = stream.read()
    ...
    def click_button9(self):
        stream = os.popen('cd /usr/src/CalypsoBTS/ && sudo python3 smS.py 111 SMSTest & echo "SMS SENT!"')
        out = stream.read()
        msg_box = QMessageBox(self)
    ...
    def handle_smpp_spam(self, dialog):
        ...
        cmd = f'cd /usr/src/CalypsoBTS/ && sudo python3 smpp.py {sender} "{message}"'
        stream = os.popen(cmd)
        out = stream.read()
    ...
    def handle_ussd_spam(self, dialog):
        message = dialog.message_edit.text()
        subs = self.get_subscribers()
        cmd = f'cd /usr/src/CalypsoBTS/ && sudo python3 ussd.py 1 "{message}"'
        stream = os.popen(cmd)
        out = stream.read()
    ...
    def handle_sms_ddos(self, dialog):
        ...
        message = dialog.message_edit.text()
        cmd = f'cd /usr/src/CalypsoBTS/ && sudo python3 sms_attack.py {number} {count} "{message}"'
    ...
    def handle_spoof_msisdn(self, dialog):
        ...
        cmd = f'cd /usr/src/CalypsoBTS/ && sudo python3 msisdn_changer.py {target_id} {spoof_number}'
        stream = os.popen(cmd)
        out = stream.read()
        msg_box = QMessageBox(self)
